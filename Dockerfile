...
RUN dnf install  --setopt=install_weak_deps=False --best -y rpm-build glibc-devel gtk2-devel libX11.so.6 libatk-1.0.so.0 libcairo.so.2 libgdk-x11-2.0.so.0 libgdk_pixbuf-2.0.so.0 libgtk-x11-2.0.so.0 libpango-1.0.so.0 libpango-1.0.so.0 perl desktop-file-utils 
